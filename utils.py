...
# Returns a one dimensional tensor of indices that contains the max value of of each row in the 2d array
def get_argmax_tensor(y_h):
    return torch.argmax(y_h, dim=1)

# Returns the boolean difference vector between y_h, y
def loss_equalities(y_h, y):
    return y_h == y
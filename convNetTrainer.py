...
def compute_loss_and_accuracy(
    ...
    with torch.no_grad():
        for (X_batch, Y_batch) in dataloader:
            ...
            # Accuracy
            argMaxIndicesTensor = utils.get_argmax_tensor(output_probs)
            lossVector = utils.loss_equalities(argMaxIndicesTensor, Y_batch)
            accuracyCount += lossVector.int().sum()

            # Loss
            average_loss += loss_criterion(output_probs, Y_batch)
            iterations += 1

        accuracy = (100 * accuracyCount) / (dataloader.batch_size * iterations)
...
class ExampleModel(nn.Module):
    ...
    def __init__(self,
        ...
        # A couple of these variables spans two filters (naming is not quite correct)
        num_filters = 32  # Set number of filters in first conv layer
        num_filters_second_layer = 128
        num_filters_third_layer = 256
        num_filters_fourth_layer = 256
        num_filters_fifth_layer = 512

        num_nodes_fully_connected = 64
        ...
        self.feature_extractor = nn.Sequential(
            ...
            nn.Conv2d(
                ...
                out_channels=num_filters_third_layer,
                kernel_size=3,
                stride=1,
                padding=1
            ),
            ...
            nn.Conv2d(
                in_channels=num_filters_third_layer,
                out_channels=num_filters_third_layer,
                kernel_size=3,
                stride=1,
                padding=1
            ),
            nn.BatchNorm2d(num_filters_third_layer),
            nn.ReLU(),
            ...
            nn.Conv2d(
                in_channels=num_filters_third_layer,
                out_channels=num_filters_fourth_layer,
                kernel_size=3,
                stride=1,
                padding=1
            ),
            nn.BatchNorm2d(num_filters_fourth_layer),
            nn.ReLU(),

            nn.Conv2d(
                in_channels=num_filters_fourth_layer,
                out_channels=num_filters_fifth_layer,
                kernel_size=3,
                stride=1,
                padding=1
            ),
    ...
    def forward(self, x):
        ...
        # Flatten image before sending it to the fully connected layer
